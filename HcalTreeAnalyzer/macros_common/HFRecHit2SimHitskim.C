...
void HFRecHit2SimHitskim(const char *infile1, const char *infile2, const char *outfile, const char *flag1 = "single", const char *flag2 = "single", const char *process = "full"){
  ...
  while(tReader1->Next()&&continueLoop){
   ...
   if(debug) cout << "Processing event: " << tree1Events << ", " << *runID1 << ":" << *lumiID1 << ":" << *eventID1 << endl;
	...
	if(rmatched || (partialMatch != 0)){
		...
		if((En1 <= 5)&&(En2 >= 26)){
			double otherEn = (*readRecHitsHF_En2).at(v2BookMark);
			int otherDepth = (*readRecHitsHF_depth2).at(v2BookMark);

			std::cout << *runID1 << ":" << *lumiID1 << ":" << *eventID1
				<< " ieta: " << ieta1 << " iphi: " << iphi1 << " depth: " << depth1
				<< " RecHit: " << En1;
			if(partialMatch != 0) std::cout << " SH Depth " << partialMatch << ": " << runningSum;
			if(rmatched) std::cout << " SH Depth " << otherDepth << ": " << otherEn;
			if((partialMatch==0)||!rmatched) std::cout << " Only One SimHit!";
			std::cout << " SH sum: " << En2;
			std::cout << std::endl;
